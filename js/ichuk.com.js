...
var iChukCore = {
	Inital:function(){
		...
		cc.AutoSplitContent = function(father,son,minuse){
			var fathersize = $(father).size();
		    if(fathersize>0)
			{
			    for(var x = 0;x < fathersize;x++)
				{
				    var item = $(father).eq(x);
					var w = item.width();
					var items = item.find(son);
					if(items.size() > 0)
					{
					    var splitwidth = w/Number(items.size()); 
						splitwidth = (typeof(minuse) == "number")?splitwidth-minuse:splitwidth;
						items.width(splitwidth);
					}
				}
			}
		}
		...
		* 图标选择器
		* handler->$("#handler") 
		* callback 若有回掉，则将数据返回
		*/
		cc.iChukIconChooser = function (handler,callback){
			var iconHandler = handler; 
			var title = "图标选择器";
			var iconList;
			iconList = [
				           "icon-home","icon-message","icon-comments","icon-search","icon-refresh","icon-more","icon-exit","icon-setting","icon-blacklist","icon-atme","icon-remind","icon-chat","icon-data","icon-friends","icon-write","icon-call-in","icon-dial","icon-delete","icon-location","icon-drafts","icon-read","icon-privace","icon-about","icon-security","icon-complete","icon-reply","icon-history","icon-back","icon-experience","icon-hung-off","icon-topic","icon-theme","icon-weibo","icon-call","icon-trash","icon-call-out","icon-feedback","icon-grade","icon-money","icon-app","icon-follow","icon-followed","icon-attention","icon-add","icon-close",
			           ]; 
			iconHandler.click(function(){
				var html = "<div class=\'ichuk-icon-selector-wrap\'>\
				                <ul style=\'overflow:hidden;\' class=\'ichuk-icon-selector\'>";
				for(var x=0;x < iconList.length;x++)
				{
					html += "<div style=\'font-weight:200;margin:5px;padding:5px;text-align: center;color: gray;float: left;cursor:pointer;border:1px solid #ccc;\' class=\'ichuk-icon-selector-item\' data-icon=\'"+iconList[x]+"\'>";
					html += "       <div class=\'ichuk-icon "+iconList[x]+"\'></div>";
					html += "       <div class=\'selector-info\'>"+iconList[x]+"</div>";
					html += "</div>";
				}
				html += "       </ul>\
				            </div>";
				var info = cc.InitDialogFrame(title,html); 
				$("#"+info.content).find(".ichuk-icon-selector-wrap").find(".ichuk-icon-selector-item").click(function(){
					var icon = $(this).attr("data-icon");
					if(typeof(callback) == "function"){
						callback("ichuk-icon "+icon);
					}
					info.close();
				})
				$("#"+info.content).find(".ichuk-icon-selector-wrap").find(".ichuk-icon-selector-item").mouseover(function(){
  $(this).find(".ichuk-icon").addClass("on");
});
				$("#"+info.content).find(".ichuk-icon-selector-wrap").find(".ichuk-icon-selector-item").mouseout(function(){
  $(this).find(".ichuk-icon").removeClass("on");
});
		...
		cc.GenSideMenu = function (menu,route){
			var menu_object = eval("("+menu+")");
			
			var html = "";
			for(var i = 0;i < menu_object.list.length ;i++)
			{
				var selected = Number(menu_object.list[i].selected);
				if(selected)
				{
					html += '<div class=\"section\">\
								<ul class=\"section_title\">\
									'+menu_object.list[i].section+'\
								</ul>';
					for(var x =0 ;x < menu_object.list[i].lists.length;x ++)
					{
						var iselected = Number(menu_object.list[i].lists[x].selected);
						if(iselected)
						{
							var icon = ( typeof(menu_object.list[i].lists[x].icon) != "undefined" && menu_object.list[i].lists[x].icon != "")?menu_object.list[i].lists[x].icon:menu_object.list[i].lists[x].content;
							var href = (menu_object.list[i].lists[x].type == 'link')?'../../?'+route+'/'+menu_object.list[i].lists[x].content+'/':((menu_object.list[i].lists[x].type == 'href')?menu_object.list[i].lists[x].link:menu_object.list[i].lists[x].content);
							html += '<ul class=\"section_content\">\
										<a class=\"section_item '+menu_object.list[i].lists[x].content+'\" href=\"'+href+'\">\
											<i class=\"ichuk-icon icon-'+icon+'\">\
												\
											</i>\
											<span>\
												'+menu_object.list[i].lists[x].name+'\
											</span>\
										</a>\
									</ul>';
						}
			...
			$(".sysinfo").html("version-menu-data / "+menu_object.version);
			$(".side_menu").html(html);
		}
		cc.InitProgress = function(target){
			for(var x = 0;x < target.size();x++)
			{
				var first = target.eq(x).find("ul").eq(0);
				var last = target.eq(x).find("ul").eq(1);
				var width = target.eq(x).width();
				var total = target.eq(x).attr("data-total");
				var count = target.eq(x).attr("data-count");
				var percent = (count/total).toFixed(2);
				var show = percent*width;
				target.eq(x).attr("title",percent*100+"%");
				target.eq(x).css({
				    "cursor":"pointer"
				})
				last.animate({width:show},'normal');
			}
		}
		cc.JsExchangeTime = function(str_time){
        ...
        /*
		* 初始化开关形式按钮
		* InitOnOff(object,callback,defaultvalue);
		* defaultvalue 默认值，若赋予默认值，将主动寻值
		* <div id="all_course_flag" class="onoff " tar="all_course_flag">
		*    <ul val="1">
		*		 有
		*    </ul>
		*    <ul val="0" class="on">
		...
		* </div>
		*/
		cc.InitOnOff = function(object,callback,defaultvalue){
		    var OnOffObj = $(object);
			...
			{
				$("input[name="+OnOffObj.attr("tar")+"]").val(_val);
				OnOffObj.find('ul[val='+defaultvalue+']').trigger("click");
			}
		};
		/*
		*  cc.shake($("#ID"),"red",3);
		...
		*/
		...
		cc.InitProvinceCityCounty = function(select_province_name,select_city_name,select_county_name,province,city,county)
		{
			...
			if(select_city_name != "" &&  select_city_name != undefined && select_city_name != null)
			{
			    $("select[name="+select_city_name+"]").append(defaultoption);
			}
			if(select_county_name != "" &&  select_county_name != undefined && select_county_name != null)
			{
		...
		/*
		* _postiontype_ 0->数字指示 1->圆点指示（居右）2->圆点指示（居中）
		*
		*/
		cc.InitiChukScroll = function(_targt_,_item_,_postiontype_,_arrow_show_)
		{
			if($(_item_).size() > 0)
			{
				var _scoll_ = {};
				_scoll_.index = 0;
				var positiondirection = (_postiontype_==1)?"right":"center";
				var arrowshow = (_arrow_show_==0||typeof(_arrow_show_) == "undefined")?"none":"block";
				...
				_inner_ += "<div class=\'_HANDLE_WRAP_\' style=\'display:"+arrowshow+";line-height:100px;position: absolute;top: 50%;height: 100px;margin-top: -50px;width: 100%;font-size: 50px;color: white;text-shadow: 0 0 5px #303030;font-family: monospace;\'>\
				            ...
				            <div style=\'font-family: cursive;position: absolute;bottom:0;text-align: "+positiondirection+";width: 96%;color: white;padding: 10px 2%;text-shadow: 0 0 5px #1F1F1F;\' class=\'_POSITION_\'>";
				            if(_postiontype_ == 0)
				            {
				...
				$(_targt_).find("._POSITION_POINT_").click(function(){
		...
		cc.Calendar = function(target,callback,widthtype,season,dayclick){// widthtype->宽度类型 0 设备宽度 1 父元素宽度
			...
			var html = "<div id=\'canlendar_"+rand+"\' style=\'background-color: white;font-size: 14px;border: 1px solid #ccc;width:"+_width_+"px;height:"+_height_+"px;color:gray;text-align: center;\'>\
			                ...
			                <div class=\'section_center\' style=\'height:"+section_center_height+"px;\'>\
			                    <ul style=\'border-right:1px solid #ccc;border-bottom:1px solid #ccc;font-size: 23px;float:left;height:"+(section_center_height-1)+"px;width:"+(handle_mounth_width-1)+"px;\' data-type=\'mounth\' data=\'"+time.month+"\' class=\'handle handle-mounth\'>\
	    ...
	    cc.Confirm = function(content,callback_){
		    var rand = Math.floor((Math.random()*100000000)+1);
			var html = content;
			var form;
			var callback = {};
			...
			form = cc.InitDialogFrame("",html,config);
		    return form ;
		}
		cc.Alert = function (content,title){
			var rand = Math.floor((Math.random()*100000000)+1);
			var html = content;
			var form;
			var config = {
			...
			form = cc.InitDialogFrame(title,html,config);
			setTimeout(function(){
			    form.close();
			},3000)
		    return form ;
		}
	    cc.Prompt = function(title,defaultvalue,confirmcallback,canclecallback){
			var rand = Math.floor((Math.random()*100000000)+1);
			var html = "<input style=\'width: 98%;padding: 2% 1%;color: #505050;font-size: 16px;border: 1px solid #ccc;border-radius: 2px;background-color: #f7f7f7;\' name=\'prompt_"+rand+"\' placeholder=\'键入值\' value=\'"+defaultvalue+"\'/>";
			var form;
			var callback = {};
			callback.confirm = function(){
							     form.close();
								 ...
								 {
									 var value = $("input[name=prompt_"+rand+"]").val();
			...
			$("input[name=prompt_"+rand+"]").focus();
			$("input[name=prompt_"+rand+"]").keydown(function(e){
				if(e.keyCode==13){
					callback.confirm();
				}
			})
		...
		/*
		* jsonconfig {width:"100px/100%",height:"100px/100%",confirm:1,cancle:1,confirmcallback:callback,canclecallback:callback,effect:1}
		* jsonconfig 配置参数，可为空
		* width-> 宽度，支持100px或者 100%这样的单位
		* height-> 高度，支持100px或者 100%这样的单位
		* confirm-> 1：显示确认菜单 0：不显示确认菜单
		* cancle-> 1：显示取消菜单 0：不显示确认菜单
		* confirmcallback typeof(callback)=="function" 时，则调用回调，此时默认显示确认菜单，confirm无效
		* canclecallback typeof(callback)=="function" 时，则调用回调，此时默认取消确认菜单，cancle无效
		* effect 1->淡出 飞出
		*/
		cc.InitDialogFrame = function(title,content,jsonconfig)
			{
				var effect = 1;
			    var rand = Math.floor((Math.random()*100000000)+1); 
			    var left = cc.IsMobile()?5:200;
				var padding = 10; 
				var _titleheight = 35;
				var _height = cc.DeviceHeight - left -_titleheight; 
				var _width = cc.DeviceWidth - left - padding;
				var confirmbutton = false;
				var canclebutton = false;
			    var _wrap = "dialog_wrap";
			    var _content = "dialog_content";
			    var _title = "dialog_title";
			    var _handle = "dialog_handle";
				var result = {};
				result.rand = rand;
				...
				{
				    if(typeof(jsonconfig.width)=="string")
					{
						var arr = jsonconfig.width.split("px");//单位为像素
						var arr2 = jsonconfig.width.split("%"); 
						if(arr.length ==2)
						{
						    _width = Number(arr[0]);
						}
				    ...
				    if(typeof(jsonconfig.height)=="string")
					{
						var arr = jsonconfig.height.split("px");//单位为像素
						var arr2 = jsonconfig.height.split("%"); 
						if(arr.length ==2)
						{
						    _height = Number(arr[0]);
						} 
					} 
				    if(typeof(jsonconfig.effect)=="number")
					{
						effect = (jsonconfig.effect==1)?1:2;
					}

					if(typeof(jsonconfig.confirmcallback)=="function")
					{
					    confirmbutton = true;
					}
					if(typeof(jsonconfig.canclecallback)=="function")
					{
					    canclebutton = true;
					}
				...
				title = (typeof(title)=="string"&& title!="")?title:"消息提示";
				var _marginleft_ = (cc.DeviceWidth-_width)/2;
				var _margintop_ = (cc.DeviceHeight-_height)/2; 
			    var html = "<div style=\'width: 100%;height: 100%;position: fixed;top: 0;z-index:15;background-color: rgba(0, 0, 0, 0.6);\'>\
								<div id=\'"+_wrap+"_"+rand+"\' class=\'"+_wrap+"\'>\
									...
									<div id=\'"+_handle+"_"+rand+"\' class=\'"+_handle+"\'>";
				if(confirmbutton)
				{
					html += "<a class=\'ichuk-background-blue ichuk-button "+_handle+"_confirm\'>确定</a>";
				}
				if(canclebutton)
				{
					html += "<a class=\'ichuk-background-yellow ichuk-button "+_handle+"_cancle\'>取消</a>";
				}
				html +=			   "</div>\
								</div>\
					        </div>";
				...
				if(confirmbutton)
				{
				    $("."+_handle+"_confirm").click(function(){
					    if(typeof(jsonconfig.confirmcallback)=="function")
						{
						    jsonconfig.confirmcallback();
						}
						else
						{
						    result.close();
			    ...
			    if (canclebutton)
			    {
					$("."+_handle+"_cancle").click(function(){
					    if(typeof(jsonconfig.canclecallback)=="function")
						{
						    jsonconfig.canclecallback();
						}
						else
						{
						    result.close();
				...
				result.close = function(){ 
					...
					{
						//1.淡出
						$("#"+_wrap+"_"+rand).parent().fadeOut("fast",function(){
							$(this).detach();
						});
					}
					else if(result.effect == 2){
						//2.飞出
						$("#"+_wrap+"_"+rand).parent().animate({marginLeft:"-"+_marginleft_+"px"},"fast",function(){
							$(this).detach();
						});  
					} 
				}
				...
				$("#"+_wrap+"_"+rand).animate({marginLeft:_marginleft_+"px",top:_margintop_+"px"},"fast");
				
				$("#"+_wrap+"_"+rand).css({"height":_height+'px'});
				$("#"+_wrap+"_"+rand).find("#"+_content+"_"+rand).css({"max-height":(_height -_titleheight)+'px'});
				
				$("#"+result.wrap).find("._close").click(function(){
					result.close();
				})
				return result;
			}
		...
		cc.ZanComment = function (aid,target)
		{
			if(Number(aid) > 0 )
			{
				mid = cc.CheckLogin();
				if(  Number(mid) >0)
				{
				    ...
				    postdata.mid = mid;
				    postdata.type = 3;
					var url = "../../?api/submitzan/"+cc.accesstoken+"/"+cc.openid;
			        $.ajax({
						 type: "POST",
						 url: url,
						 data:postdata,
				...
				{
					var r = confirm("您还没有登录，需要登录来操作吗？");
					if(r)
					{
						window.location.href="../../?login&uri=../?post/view/"+aid;
					} 
				}
		    }
		}
		cc.ZanArchive = function (aid,target)
		{
			if(Number(aid) > 0 )
			{
				mid = cc.CheckLogin();
		...
		cc.CommentArchive = function (aid,target,title,commentid,commentmid,commentnickname)
		{
			mid = cc.CheckLogin();
			if(  Number(mid) >0)
			{
				var html = "<div class=\'notice\' style=\'color: lightgray;font-size: 13px;text-align: center;\'>评论将由作者筛选后显示，对所有人可见。</div>\
				            <div class=\'ihavedonewrap\'>\
				                <ul>\
				                    <textarea rows=\'3\' name=\'body\' placeholder=\'写点什么吧\'></textarea>\
				                </ul>\
				                <ul>\
				                    <i class=\'submit_ihavedone\' ret=\'0\'>发表评论</i>\
				                </ul>\
				            </div>";
				var frame = cc.InitDialogFrame("评论："+title,html);
				$("#"+frame.wrap).find(".submit_ihavedone").click(function(){
					var ret = $("#"+frame.wrap).find(".submit_ihavedone").attr('ret');
					if(ret == "0")
					{
						var content = $("#"+frame.wrap).find("textarea[name=body]").val();
						//console.log(content);
						if(content != "")
						{
							var postdata ={};
						    ...
						    postdata.commentnickname = commentnickname;
						    if(Number(commentid) > 0)
						    {
							    content = '<msg><user id='+commentmid+'>%s%</user>'+title+'</msg>'+'<content>'+content+'</content>';
						    }
						    postdata.content = content;
						    $("#"+frame.wrap).find(".submit_ihavedone").attr('ret',1);
						    $("#"+frame.wrap).find(".submit_ihavedone").text("正在提交，请稍后...");
							var url = "../../?api/submitcomment/"+cc.accesstoken+"/"+cc.openid;
					        $.ajax({
								 type: "POST",
								 url: url,
								 data:postdata,
								 ...
								 success : function(data){
									...
									if(data.ret == 1)
									{
								        ...
								        var _new_ = Number(org) + 1;
								        target.text(_new_);
						                $("#"+frame.wrap).find(".submit_ihavedone").text("评论成功");
								        $("#"+frame.wrap).fadeOut(function(){
									        $(this).detach();
								        })
								        window.location.reload();
									}
									else{
						                $("#"+frame.wrap).find(".submit_ihavedone").attr('ret',0);
						                $("#"+frame.wrap).find(".submit_ihavedone").text("发表评论");
										alert(data.msg);
									}
								 },
								 error:function(data){
		...
		/*
		* yyyy-MM-dd HH:mm:ss 时间转化为 unix 时间戳
		*/
		cc.js_strto_time= function (str_time,type){
			var new_str = str_time.replace(/:/g,'-');
			new_str = new_str.replace(/ /g,'-');
			var arr = new_str.split("-");
			var datum = (type=="yyyy-MM-dd")?new Date(Date.UTC(arr[0],arr[1]-1,arr[2],0,0,0)):new Date(Date.UTC(arr[0],arr[1]-1,arr[2],arr[3]-8,arr[4],arr[5]));
			return strtotime = datum.getTime()/1000;
		}
