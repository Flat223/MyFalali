define(function(require){
	...
	var skuTableW = 600.0;//库存信息table的宽度
	var maxPropVals = 6;
// 	var maxPropSize = 3;
	var counter = 0;
	var props = new Map();
	...
	var handlePro = {
		init:function(){
			...
			$.ajax({
				...
				success:function(data){
					if(data.ret == 1){
						storeProps = data.pro_vals;
						propCount = data.property.length;
						counter = data.pro_vals.length;
						
						if(storeProps.length > 0){
							handlePro.initProductProperty();
							handlePro.initSkuStatus();
						}
                    } else {
		...
		initProductProperty(){			
			for(var i in storeProps){
				var propvalMap = storeProps[i];
				var prop = {};
				prop.propertyid = propvalMap.id;
				prop.name = propvalMap.name;
				
				var propvals = "";
				var propvalArray = propvalMap.vals;
				for(var j in propvalArray){
					var propval = propvalArray[j];
					if(propvals == ""){
						propvals = propval.name;
					} else {
						propvals = propvals + "," + propval.name;
					}
				}
				prop.propvals = propvals;
				i++;
				var key = "prop" + i;
				props.put(key,prop);
			}
		...
		//初始化库存状态
		initSkuStatus:function(){
			skus = handlePro.combinationArr(storeProps);
			var newSkuids = [];
			var restSkuids = [];
			for(var i in skus){
			...
			if(storeProps.length > 3){
				var width = skuTableW;
				width = width + parseFloat((storeProps.length - 3)*60.0)+'px';
...
*/
		...
		modProp:function(id){
			...
			$("#prop_stat").val("mod");
			$("#propid").val(id);
			$("input[name=pro_name]").val(prop.name).focus();
			if(prop.propvals != ""){
				var arr = prop.propvals.split(",");
				var html = "";
				for(var i in arr){
...
// 				html += "<button type='button' class='btn btn-primary prop_del' style='padding:5px 25px;margin:0 5px;'>删除</button></td></tr>";
				...
				if($(".noProval_hint").css('display') == "block"){
					$(".provalTable").show();
					$(".noProval_hint").hide();
				}
			}else{
			...
			$.ajax({
				...
				success:function(data){
					...
					if(dataobj.ret == 1){
						var params = {};
						params.title = '提示';	
						params.msg = dataobj.msg;	
						params.needCancelBtn = false;
