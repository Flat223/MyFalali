	...
	var handle = {
		...
		init:function(){
			...
			$("#login,#regist").on("click",function(){
				...
				if(id == "login"){
					$("#tab>ul>li[ref='quick_login']").click();
				}else{
					$("#tab>ul>li[ref='quick_regist']").click();
				}
			});
			...
			$(".head_search_btn").on("click",function(){
				var val = $.trim($(".header_search_val").val());
				window.open(getHost()+"/search.html?search="+val);
			});
			$(".header_search_val").keypress(function(e){
				if(e.keyCode == 13){
