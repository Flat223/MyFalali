	...
	var handle = {
		init:function(){
			...
			$("#pay").on("click",function(){
				...
				layer.confirm('充值元宝：'+coin+"<br/>充值金额："+parseFloat(money)+"<br/>充值方式："+type, {
					title:"温馨提示",
					btn: ['确认','取消'], //按钮
					offset:['220px','40%']
				}, function(index){
		...
		pay:function(pay_money_type,pay_type){
			window.location.href = getHost()+"/user/recharge.html?rechargeType=2"+"&payType="+pay_type+"&rechargeOption="+pay_money_type;
		}
	};
